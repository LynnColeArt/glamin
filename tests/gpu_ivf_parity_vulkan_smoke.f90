program glamin_gpu_ivf_parity_vulkan_smoke
  ...
  query_data = [0.2_real32, 0.1_real32, 2.1_real32, 2.0_real32]
  ...
  if (status /= GLAMIN_OK) error stop "ivf_create failed"
  call ivf_train(index, vectors, status)
  if (status /= GLAMIN_OK) error stop "ivf_train failed"
  call ivf_add(index, vectors, status)
  if (status /= GLAMIN_OK) error stop "ivf_add failed"

  call gpu_clear_backend(status)

  cpu_distances = VectorBlock()
  cpu_labels = VectorBlock()
  call ivf_search(index, queries, result_k, 1_int32, cpu_distances, cpu_labels, status)
  if (status /= GLAMIN_OK) error stop "ivf_search cpu failed"

  result_count = queries%length * result_k
  call c_f_pointer(cpu_labels%data, cpu_label_ptr, [int(result_count)])
  if (any(cpu_label_ptr /= [1_int32, 2_int32, 3_int32, 4_int32])) then
    error stop "cpu label mismatch"
  end if
  ...
  call ivf_search(index, queries, result_k, 1_int32, gpu_distances, gpu_labels, status)
  if (status /= GLAMIN_OK) error stop "ivf_search gpu failed"
  ...
  write (*, '(a)') 'gpu ivf parity vulkan ok'
end program glamin_gpu_ivf_parity_vulkan_smoke
