program glamin_gpu_ivf_parity_smoke
  ...
  call ivf_search(index, queries, k, 1_int32, cpu_distances, cpu_labels, status)
  if (status /= GLAMIN_OK) error stop "ivf_search cpu failed"

  result_count = queries%length * k
  ...
  call ivf_search(index, queries, k, 1_int32, gpu_distances, gpu_labels, status)
  ...
  write (*, '(a)') 'gpu ivf parity ok'
end program glamin_gpu_ivf_parity_smoke
