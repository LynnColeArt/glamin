program glamin_gpu_distance_parity_smoke
  ...
  if (trim(backend_name) /= 'cuda') error stop "unexpected backend"
  ...
  write (*, '(a)') 'gpu distance parity ok'
end program glamin_gpu_distance_parity_smoke
