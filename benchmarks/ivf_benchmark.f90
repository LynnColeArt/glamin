program glamin_ivf_benchmark
  use iso_fortran_env, only: int32, int64, real32, real64
  use iso_c_binding, only: c_f_pointer, c_ptr, c_size_t
  use glamin_errors, only: GLAMIN_OK
  use glamin_index_ivf, only: IvfIndex, ivf_add, ivf_create, ivf_destroy, ivf_search, &
    ivf_train
  ...
  call read_arguments(dim, vector_count, query_count, nlist, nprobe, k, iterations)
  ...
  call ivf_create(index, dim, nlist, METRIC_L2, status)
...
contains
  subroutine run_search(index, queries, nprobe, k, iterations, distances, labels)
    ...
    do iter = 1, iterations
      call ivf_search(index, queries, k, nprobe, distances, labels, status)
      if (status /= GLAMIN_OK) error stop "ivf_search failed"
    end do
  ...
  subroutine read_arguments(dim, vector_count, query_count, nlist, nprobe, k, iterations)
...
end program glamin_ivf_benchmark
