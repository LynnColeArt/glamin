program glamin_distance_benchmark
  ...
  if (status /= GLAMIN_OK) error stop "L2 warmup failed"
  call free_aligned(distances%data, free_status)

  call run_benchmark(.true., queries, vectors, iterations, "L2")
  call run_benchmark(.false., queries, vectors, iterations, "IP")
  ...
  subroutine run_benchmark(use_l2, queries, vectors, iterations, label)
    ...
    do iter = 1, iterations
      ...
      if (status /= GLAMIN_OK) then
        error stop "Distance kernel failed"
      end if
    end do
...
end program glamin_distance_benchmark
